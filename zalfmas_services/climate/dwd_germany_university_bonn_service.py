...
async def main(config):
    ...
    if "config_toml" in args:
        with open(args.config_toml, "r") as f:
            toml_config = tk.load(f)
    else:
        toml_config = {}
    config.update(toml_config.get("service", {}))
    config.update(toml_config.get("caps", {}))
    config.update(toml_config.get("network", {}))
    ...
    service = ccdi.Service(meta_plus_data,
                           id=config["id"],
                           name=config.get("name", "service name here"),
                           description=config.get("description", "description here"),
                           restorer=restorer)
...
#
#     if config_toml:
#         with open(config_toml, "rt") as _:
#             config_toml_str = _.read()
...
#         config_toml_str = ""
    ...
    asyncio.run(capnp.run(main(default_config)))
    #    path_to_data="/run/user/1000/gvfs/sftp:host=login01.cluster.zalf.de,user=rpm/beegfs/common/data/climate/dwd/csvs/",
