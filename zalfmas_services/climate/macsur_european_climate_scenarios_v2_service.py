...
from capnproto_schemas import (
    common_capnp,
    geo_coord_capnp as geo_capnp,
    climate_data_old_capnp as climate_data_capnp,
)
...
def lat_lon_interpolator():
    ...
    if not hasattr(lat_lon_interpolator, "interpol"):
        lat_lon_interpolator.interpol = (
            create_lat_lon_interpolator_from_csv_coords_file(
                "macsur_european_climate_scenarios_geo_coords_and_altitude.csv"
            )
        )
...
class Station(climate_data_capnpData.Station.Server):
        ...
        if len(self._time_series) == 0:
            for scen in self._sim.scenarios:
                for real in scen.realizations:
                    for ts in real.closest_time_series_at(
                        self._geo_coord["lat"], self._geo_coord["lon"]
                    ):
    ...
    def timeSeriesFor(
        self, scenarioId, realizationId, **kwargs
    ):  # (scenarioId :Text, realizationId :Text) -> (timeSeries :TimeSeries);
        # get all time series for a given scenario and realization at this station
        return list(
            filter(
                lambda ts: ts.scenarioInfo().id == scenarioId
                and ts.realizationInfo().id == realizationId,
                self.allTimeSeries(),
            )
        )
...
class TimeSeries(climate_data_capnpData.TimeSeries.Server):
    def __init__(
        ...
        time_range_id=None,
        dataframe=None,
        adapt_timeseries_to_future_dates=None,
    ):
        ...
        if path_to_csv and not time_range_id:
            raise Exception(
                "Missing argument, when supplying the path_to_csv, you also have to supply a time_range_id!"
            )
    ...
    @classmethod
    def from_csv_file(
        cls, realization, path_to_csv, time_range_id, adapt_timeseries_to_future_dates
    ):
        return TimeSeries(
            realization,
            path_to_csv,
    ...
    def dataframe(self):
        ...
        if self._df is None and self._path_to_csv:
            ...
            if self._time_range_id != "0" and self._adapt_timeseries_to_future_dates:
                ...
                self._df.set_index(
                    pd.date_range(
                        date(time_range["from"], 1, 1), date(time_range["to"], 12, 31)
                    ),
                    inplace=True,
                )
    ...
    def range_context(self, context):  # -> (startDate :Date, endDate :Date);
        context.results.startDate = create_capnp_date(
            date.fromisoformat(str(self.dataframe.index[0])[:10])
        )
        context.results.endDate = create_capnp_date(
...
class Simulation(climate_data_capnpData.Simulation.Server):
    ...
    def stations(self, **kwargs):  # () -> (stations :List(Station));
        return list(
            [
                Station(self, "[r:{}/c:{}]".format(row_col[0], row_col[1]), coord)
...
class Realization(climate_data_capnpData.Realization.Server):
    ...
    def closest_time_series_at(self, lat, lon):
        ...
        for time_range in c["time_ranges"]:
            formated_path = c["path_template"].format(
                sim_id=c["sim_id"],
                scen_id=c["scen_id"],
                version=c["version_id"],
                period_id=time_range,
                row=row,
                col=col,
            )
            closest_time_series.append(
                TimeSeries.from_csv_file(
                    self,
                    formated_path,
                    time_range,
                    self._adapt_timeseries_to_future_dates,
                )
            )
    ...
    def closestTimeSeriesAt(
        self, latlon, **kwargs
    ):  # (latlon :Geo.LatLonCoord) -> (timeSeries :List(TimeSeries));
...
class Service(climate_data_capnpData.Service.Server):
    ...
    def getAvailableSimulations_context(
        self, context
