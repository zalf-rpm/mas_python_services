...
common_capnp = capnp.load(
    str(PATH_TO_CAPNP_SCHEMAS / "common.capnp"), imports=abs_imports
)
...
class Registry(reg_capnp.Service.Registry.Server):
    ...
    def getAvailableServices_context(
        self, context
        ...
        if context.params.which() == "all":
            ...
            for service_entry in filter(
                lambda v: v["entry"]["type"] == serviceType, self._services.values()
            ):
                all_services.append(service_entry["entry"])

        context.results.services = all_services

    def getService_context(
        self, context
    ):  # getService [Service] @1 (regToken :Text) -> (service :Service);
        regToken = context.params.regToken
        if regToken in self._services:
            return self._service[regToken]["entry"]["service"]

    def registerService_context(
        self, context
    ):  # registerService @2 (type :Type, service :Common.Identifiable) -> (regToken :Text, unreg :Common.Registry.Unregister);
        ps = context.params
        serviceType = ps.type
        regToken = str(uuid.uuid4())
        unreg = Unregister(regToken, lambda: self._services.pop(regToken, None))
        self._services[regToken] = {
            "entry": {"regToken": regToken, "type": serviceType, "service": ps.service},
            "unreg": unreg,
        }
...
async def async_main(
    server="0.0.0.0", port=10001, id=None, name="registry name", description=None
):
    config = {
        "port": str(port),
        "server": server,
    ...
    server = await asyncio.start_server(
        new_connection_factory(registry), server, port, family=socket.AF_INET
    )
    # except Exception:
    #    print("Try IPv6")
    #    server = await asyncio.start_server(
...
def no_async_main(
    server="*", port=6003, id=None, name="registry name", description=None
):
    config = {
        "port": str(port),
        "server": server,
...
def main(
    server="*", port=10001, id=None, name=None, description=None, use_asyncio=True
):
    if use_asyncio:
        asyncio.run(
            async_main(
                server=server, port=port, id=id, name=name, description=description
            )
        )
    else:
        no_async_main(
